...
# Solver and gameplay use the same broad ESDB acceptance set. Intended starting
# words remain familiar; every accepted alternate is still included in the
# full-board audit so dictionary expansion cannot hide an easier shortcut.
...
def word_partitions(letters, cap=2000):
    ...
    def solve(rem, chosen):
        ...
        for i in options:
            ...
            if rest in dead:
                continue
            if solve(rest, chosen + (candidates[i][0],)):
                return True
        return False

    capped = solve(tuple(counts[ch] for ch in letters_used), ())
