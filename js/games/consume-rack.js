...
(function() {
  ...
  function journey() { state = null; const data = profile(), stars = data.profiles[data.active].stars, unlocked = done(stars) + 1; wrap.style.setProperty('--kt', cfg().accent); wrap.innerHTML = `<div class="kt-journey"><button data-modes>MODES</button><h1>KNOT SWAP</h1><p>${cfg().intro} Start with working groups, then use the rack tiles to make every group valid.</p><section><b>SAVE CODE</b><strong>${esc(data.active)}</strong><small>Use this public code to continue this journey on another device.</small></section><div class="kt-code"><label>ENTER A SAVE CODE</label><input id="kt-code" maxlength="12" placeholder="FROG4"><button id="kt-enter">ENTER</button></div><h2>LEVELS</h2><div class="kt-levels">${levels().map(level => { const value = stars[level.n] || 0, locked = level.n > unlocked; return `<button class="${locked ? 'locked' : ''}" data-level="${level.n}">${level.n}${value ? `<i>${'★'.repeat(value)}</i>` : ''}</button>`; }).join('')}</div></div>`; wrap.querySelector('[data-modes]').addEventListener('click', () => window.renderConsumeModes()); wrap.querySelector('.kt-levels').addEventListener('click', event => { const button = event.target.closest('[data-level]'); if (button && !button.classList.contains('locked')) start(Number(button.dataset.level)); }); wrap.querySelector('#kt-enter').addEventListener('click', () => { const code = wrap.querySelector('#kt-code').value.trim().toUpperCase(); if (!code || typeof RemoteLB === 'undefined') return; RemoteLB.lookup(mode === 'words' ? 'consume-words' : 'consume-numbers', code).then(row => { if (!row) return; const store = profile(); store.profiles[code] ||= { stars: {} }; for (let n = 1; n <= row.score; n++) store.profiles[code].stars[n] ||= 1; store.active = code; save(store); journey(); }); }); }
