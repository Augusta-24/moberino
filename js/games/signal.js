...
(function() {
  ...
  function playerDamage(amount) {
    if (signalSettings.mode === 'studio') {
      distortion = clamp(distortion + 4, 0, 100);
      burst(player.x, player.y, '#ffe61a', 8);
      return;
    }
    health -= amount || 1;
    distortion = clamp(distortion + 18, 0, 100);
    signal = clamp(signal - 8, 0, 100);
    combo = 0;
    noise(0, 0.12, 0.045, false);
    tone(190, 'sawtooth', 0, 0.16, 0.06, 70);
    burst(player.x, player.y, '#ff4444', 18);
    if (health <= 0 || distortion >= 100) finish(false);
  }
  ...
  function update(dt, t) {
    ...
    for (let i = rocks.length - 1; i >= 0; i--) {
      ...
      if (dx * dx + dy * dy <= (r.r + player.r) * (r.r + player.r)) {
        ...
        rocks.splice(i, 1);
        distortion = clamp(distortion + (signalSettings.mode === 'studio' ? 1 : 6 + Math.min(8, combo * 0.5)), 0, 100);
        signal = clamp(signal - 2, 0, 100);
        combo = 0;
        if (signalSettings.mode !== 'studio' && distortion >= 100) finish(false);
      }
    }
  ...
  function drawHud(c) {
    ...
    c.fillText('NOISE', 12, 38);
    c.fillText('HP ' + health, W - 56, 12);
    ...
    drawBar(c, 72, 39, W - 144, 10, distortion / 100, '#ff2d55');
  ...
  function showResult(won) {
    ...
    overlay.innerHTML = `
      <div class="signal-panel">
        <div class="signal-title">${won ? 'TRACK BUILT' : 'SIGNAL LOST'}</div>
        <div class="signal-subtitle">${won ? 'SAVED CHOICES READY FOR REPLAY.' : 'DISTORTION TOOK THE FIELD.'}</div>
        <div class="signal-stats">
          ...
          <div class="signal-stat">NOISE<b>${Math.round(distortion)}%</b></div>
