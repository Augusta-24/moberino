...
(function () {
  ...
  function renderPuzzle() {
    ...
    // Adjust the SVG viewBox height after the engine computed region/rack
    // positions so the bottom (rack area) is fully visible on narrow mobile
    // viewports. Defer briefly to let the engine set attributes on the SVG
    // elements it manages.
    if (ok) setTimeout(() => {
      try {
        const stageHost = document.getElementById('packing-stage');
        const svg = stageHost && stageHost.querySelector ? stageHost.querySelector('svg') : null;
        if (!svg) return;
        // compute the lowest visible y from key elements generated by engine
        let bottom = 0;
        const rackBg = document.getElementById('packing-rack-bg');
        if (rackBg) {
          const ry = parseFloat(rackBg.getAttribute('y') || '0');
          const rh = parseFloat(rackBg.getAttribute('height') || '0');
          bottom = Math.max(bottom, ry + rh);
        }
        const region = document.getElementById('packing-region');
        if (region && typeof region.getBBox === 'function') {
          const bb = region.getBBox();
          bottom = Math.max(bottom, bb.y + bb.height + 20);
        }
        ...
        svg.querySelectorAll('rect').forEach(r => {
          ...
          if (w === 560 && (h === 1120 || h === 820)) r.setAttribute('height', desired);
        });
      } catch (e) {
        // swallow — this is only a best-effort visual tweak
      }
    }, 0);
  }
